...
const PLAYER_HEIGHT = BLOCK_SIZE * 2; // 2 blocks tall
const ANIMAL_SIZE = Math.round(BLOCK_SIZE * 1.2); // about 24px
...
let gameMode; // "normal" or "hardcore"
let world = {};
let selectedBlock = null;
let cutTrees = [];
const dayDuration = 300000; // 5 minutes in ms
let gameStartTime = Date.now();
const textureCache = {};
let gameOverFlag = false;
let keys = {};
let paused = false; // when crafting UI is open
...
// ===== Noise Function (for terrain & caves) =====
...
// ===== World Generation (No Water/Sand) =====
function generateColumn(col) {
  ...
  let surfaceBlock = ground > 25 ? "rock" : "grass";
  
  for (let r = 0; r < totalRows; r++) {
    if (r < ground) colArray[r] = "air";
    else if (r === ground) colArray[r] = surfaceBlock;
    else if (r <= ground + 3) colArray[r] = surfaceBlock === "grass" ? "dirt" : "stone";
...
const player = {
  ...
  vy: 0,
  onGround: false,
  x: (Math.floor(Math.random() * 50) - 25) * BLOCK_SIZE,
  y: 0,
  health: 100,
  maxHealth: 100,
  invuln: 0
...
let inventory = {
  grass: 0, dirt: 0, stone: 0, wood: 0, leaves: 0, cobblestone: 0,
  flower: 0, coal: 0, "iron ingot": 0, "gold ingot": 0, diamond: 0,
  brick: 0, fence: 0, glass: 0, "mossy stone": 0,
  "crafting table": 0,
  meat: 0
};
const placeableBlocks = ["grass", "dirt", "stone", "wood", "leaves", "cobblestone", "flower", "brick", "glass", "fence", "mossy stone", "crafting table"];
...
function updateInventoryUI() {
  ...
  document.getElementById("toolStatus").innerHTML = `Tool: ${currentTool} | Health: ${player.health}`;
}
...
canvas.addEventListener("click", (e) => {
  ...
  if (blockType === "air" && selectedBlock && inventory[selectedBlock] > 0) {
    if (selectedBlock !== "crafting table" && !nonCraftableBlocks.includes(selectedBlock)) {
      if (!hasCraftingTableInWorld()) {
        showMessage("You must build a crafting table first to build tools and accessories!");
        return;
      }
    }
  ...
  if (Math.abs(blockCenterX - playerCenterX) <= breakDistance && Math.abs(blockCenterY - playerCenterY) <= breakDistance) {
    if (blockType === "wood" || blockType === "leaves") {
      ...
      if (world[col][groundRow] === "grass") cutTrees.push({ col, row: groundRow, timestamp: Date.now() + 60000 });
    }
    world[col][row] = "air";
    world[col].modified = true;
...
document.getElementById("hardcoreButton").addEventListener("click", () => startGame("hardcore"));
